...
@Slf4j
@Component
public class ApprovalIntelligenceParser {

    private final ObjectMapper objectMapper;

    public ApprovalIntelligenceParser(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public ApprovalIntelligenceReport parse(final String rawText) {
        ...
        try {
            return objectMapper.readValue(cleaned, ApprovalIntelligenceReport.class);
        } catch (final Exception exception) {
            log.error(
                    "Failed to parse raw text as ApprovalIntelligenceReport: {}",
