...
@Slf4j
@Component
public class ExecutiveDashboardParser {

    private final ObjectMapper objectMapper;

    public ExecutiveDashboardParser(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public ExecutiveDashboardReport parse(final String rawText) {
        ...
        try {
            return objectMapper.readValue(cleaned, ExecutiveDashboardReport.class);
        } catch (final Exception exception) {
            log.error(
                    "Failed to parse raw text as ExecutiveDashboardReport: {}",
