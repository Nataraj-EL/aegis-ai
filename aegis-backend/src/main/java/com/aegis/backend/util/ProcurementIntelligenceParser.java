...
@Slf4j
@Component
public class ProcurementIntelligenceParser {

    private final ObjectMapper objectMapper;

    public ProcurementIntelligenceParser(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public ProcurementIntelligenceReport parse(final String rawText) {
        ...
        try {
            return objectMapper.readValue(cleaned, ProcurementIntelligenceReport.class);
        } catch (final Exception exception) {
            log.error(
                    "Failed to parse raw text as ProcurementIntelligenceReport: {}",
