...
@Slf4j
@Component
public class KnowledgeIntelligenceParser {

    private final ObjectMapper objectMapper;

    public KnowledgeIntelligenceParser(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public KnowledgeIntelligenceReport parse(final String rawText) {
        ...
        try {
            return objectMapper.readValue(cleaned, KnowledgeIntelligenceReport.class);
        } catch (final Exception exception) {
            log.error(
                    "Failed to parse raw text as KnowledgeIntelligenceReport: {}",
