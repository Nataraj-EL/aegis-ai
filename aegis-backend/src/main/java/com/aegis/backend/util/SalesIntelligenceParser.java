...
@Slf4j
@Component
public class SalesIntelligenceParser {

    private final ObjectMapper objectMapper;

    public SalesIntelligenceParser(final ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public SalesIntelligenceReport parse(final String rawText) {
        ...
        try {
            return objectMapper.readValue(cleaned, SalesIntelligenceReport.class);
        } catch (final Exception exception) {
            log.error(
                    "Failed to parse raw text as SalesIntelligenceReport: {}",
